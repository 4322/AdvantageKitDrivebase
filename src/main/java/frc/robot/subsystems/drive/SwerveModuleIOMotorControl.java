...
public class SwerveModuleIOMotorControl implements SwerveModuleIO {
    ...
    private double[] feedForwardVolts = DriveConstants.Drive.FeedForward.voltsOverMetersPerSecAtSpeedThresholds.clone();
    ...
    @Override
    public void setDriveVoltage(double desiredMotorMetersPerSec) {
      double desiredMetersPerSecAbs = Math.abs(desiredMotorMetersPerSec);
      int i;
      
      // If motor m/s less than element 1 of FF speed threshold, i defaults to 0
      ...
      for (i = feedForwardMetersPerSecThreshold.length - 1; i > 0; i--) { 
        if (desiredMetersPerSecAbs >= feedForwardMetersPerSecThreshold[i]) {
          break;
        }
      }
      ...
      if (i == feedForwardMetersPerSecThreshold.length - 1) {
        ...
        double slope = (feedForwardVolts[lastElement] - feedForwardVolts[secondToLastElement]) / 
                        (feedForwardMetersPerSecThreshold[lastElement] - feedForwardMetersPerSecThreshold[secondToLastElement]);

        calcFeedForwardVoltsOverMetersPerSec = slope * (desiredMetersPerSecAbs - feedForwardMetersPerSecThreshold[lastElement]) 
      ...
      // Calculated through weighted average.
        ...
        double weight = (desiredMetersPerSecAbs - feedForwardMetersPerSecThreshold[upperBound]) /
                    (feedForwardMetersPerSecThreshold[lowerBound] - feedForwardMetersPerSecThreshold[upperBound]);
        
        calcFeedForwardVoltsOverMetersPerSec = (weight * feedForwardVolts[lowerBound]) + 
      ...
      calcFeedForwardVoltsOverMetersPerSec = calcFeedForwardVoltsOverMetersPerSec * Math.signum(desiredMotorMetersPerSec);

      // convert speed to volts while accounting for volts required to overcome static friction
      desiredVolts = (kSVolts * Math.signum(desiredMotorMetersPerSec)) + (calcFeedForwardVoltsOverMetersPerSec * desiredMetersPerSecAbs);
